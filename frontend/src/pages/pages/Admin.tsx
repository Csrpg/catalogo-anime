...
import { Usuario, Estadisticas } from '../types'
import { getEstadisticasApi, getUsuariosApi, cambiarRolApi, toggleActivarApi, eliminarUsuarioApi } from '../services/api'
...
const Admin = () => {
  ...
  const cambiarRol = async (id: string, rolActual: string) => {
    ...
    setUsuarios(prev => prev.map(u => u._id === id ? { ...u, rol: nuevoRol as 'admin' | 'usuario' } : u))
  }

  const toggleActivar = async (id: string) => {
    await toggleActivarApi(id)
    setUsuarios(prev => prev.map(u => u._id === id ? { ...u, activo: !u.activo } : u))
  }
  ...
  return (
    <div style={{ display: 'grid', gridTemplateColumns: '210px 1fr', minHeight: '100vh', paddingTop: '64px' }}>

      {/* Sidebar */}
      <aside style={{ background: '#0a0002', padding: '2rem 0', borderRight: '1px solid rgba(143,10,28,0.15)' }}>
        <div style={{ fontFamily: "'Cinzel', serif", fontSize: '0.56rem', letterSpacing: '0.3em', textTransform: 'uppercase', color: 'var(--primary)', padding: '0 1.5rem', marginBottom: '1.5rem' }}>
          Panel Admin
        </div>
        <ul style={{ listStyle: 'none' }}>
          {([['estadisticas', '⊞ Estadísticas'], ['usuarios', '◉ Usuarios']] as [Seccion, string][]).map(([key, label]) => (
            <li key={key}>
              <button onClick={() => setSeccion(key)} style={{
                ...
                color: seccion === key ? 'white' : 'rgba(255,255,255,0.28)',
                background: seccion === key ? 'rgba(143,10,28,0.18)' : 'transparent',
                borderLeft: `2px solid ${seccion === key ? 'var(--primary)' : 'transparent'}`,
                border: 'none', borderLeft: `2px solid ${seccion === key ? 'var(--primary)' : 'transparent'}`,
          ...
          <>
            ...
            {stats && (
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: '1.5rem' }}>
                {([['Animes', stats.totalAnimes], ['Usuarios', stats.totalUsuarios], ['Reseñas', stats.totalResenas], ['Favoritos', stats.totalFavoritos]] as [string, number][]).map(([label, val]) => (
                  <div key={label} style={{ padding: '1.5rem', background: 'white', border: '1px solid rgba(143,10,28,0.07)', borderTop: '3px solid var(--primary)' }}>
                    <div style={{ fontFamily: "'Cinzel', serif", fontSize: '2.4rem', fontWeight: 900, color: 'var(--primary)', lineHeight: 1, marginBottom: '0.3rem' }}>{val}</div>
                    <div style={{ fontFamily: "'Cinzel', serif", fontSize: '0.56rem', letterSpacing: '0.15em', textTransform: 'uppercase', color: 'var(--text)', opacity: 0.42 }}>{label}</div>
                  </div>
                ))}
              </div>
            )}
                ...
                <thead>
                  <tr>
                    {['Nombre', 'Email', 'Rol', 'Estado', 'Registrado', 'Acciones'].map(h => (
                      <th key={h} style={{ fontFamily: "'Cinzel', serif", fontSize: '0.56rem', letterSpacing: '0.15em', textTransform: 'uppercase', color: 'var(--text)', opacity: 0.42, textAlign: 'left', padding: '0.75rem 1rem', borderBottom: '1px solid rgba(143,10,28,0.12)' }}>
                        {h}
                      </th>
                    ))}
                  </tr>
                  ...
                  {usuarios.map(u => (
                    <tr key={u._id} style={{ opacity: u.activo ? 1 : 0.5 }}>
                      ...
                      <td style={{ padding: '0.85rem 1rem', borderBottom: '1px solid rgba(143,10,28,0.06)' }}>
                        <span style={{ fontFamily: "'Cinzel', serif", fontSize: '0.5rem', letterSpacing: '0.1em', textTransform: 'uppercase', padding: '0.25rem 0.6rem', background: u.rol === 'admin' ? 'var(--primary)' : 'rgba(143,10,28,0.08)', color: u.rol === 'admin' ? 'white' : 'var(--primary)' }}>
                          {u.rol}
                        </span>
                      </td>
                      <td style={{ padding: '0.85rem 1rem', borderBottom: '1px solid rgba(143,10,28,0.06)' }}>
                        <span style={{ fontFamily: "'Cinzel', serif", fontSize: '0.5rem', letterSpacing: '0.1em', textTransform: 'uppercase', padding: '0.25rem 0.6rem', background: u.activo ? 'rgba(0,120,0,0.1)' : 'rgba(200,0,0,0.1)', color: u.activo ? '#2a7a2a' : '#c00' }}>
                          {u.activo ? 'Activo' : 'Inactivo'}
                        </span>
                      </td>
                      <td style={{ padding: '0.85rem 1rem', fontFamily: "'Crimson Pro', serif", fontSize: '0.85rem', color: 'var(--text)', opacity: 0.45, borderBottom: '1px solid rgba(143,10,28,0.06)' }}>
